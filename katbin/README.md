...
3. Create a `.env` file and configure the environment variables:
   ```
   DATABASE_URL=postgres://postgres:<db_password>@katbin-db:5432/katbin
   SECRET_KEY_BASE=<paste_generated_secret_here>
   SWOOSH_USERNAME=your_email@example.com
   SWOOSH_PASSWORD=<smtp_password>
   SWOOSH_SMTP_RELAY=smtp.your-email-provider.com
   ```
   Replace placeholders with deployment-specific values. These settings are intentionally not baked into the image.
